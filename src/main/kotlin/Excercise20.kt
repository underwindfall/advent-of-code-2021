private val input = getInputAsTest("20")
...
data class InputImage(val pixels: Map<Vec2, Pixel>) {
  ...
  fun countLit(): Int {
    return pixels.count { it.value.lit }
  }
...
private fun part1() {
  ...
  repeat(2) { curr = curr.enhance(it % 2 == 1) }
  curr.debugPrint()
  println("Part1 ${curr.countLit()}")
}
...
private fun part2() {
  ...
  var curr = InputImage(m)
  repeat(50) { curr = curr.enhance(it % 2 == 1) }
  curr.debugPrint()
  println("Part1 ${curr.countLit()}")
}
